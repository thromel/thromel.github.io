...
\begin{document}
    ...
    \section{Experience}
        ...
        \begin{onecolentry}
            \begin{highlights}
                \item Backend Engineer at the KPI Library team, developing microservices-based healthcare applications handling millions of patient records using .NET Core and C\#
                \item Optimized microservices architecture with MongoDB caching and redesigned dashboard loading flows, achieving 30\% query response time reduction and 40\% loading time improvement
                \item Developed JSON-based dynamic dashboard creation powered by LLMs, enabling natural language dashboard generation with 85\% reduction in manual processing time
                \item Implemented distributed tracing with Jaeger, comprehensive testing (72\% to 95\% coverage), and CI/CD pipeline optimizations reducing build times by 40\%
                \item Architected cloud-native applications on AWS using Lambda, EC2, S3, DynamoDB, and implemented zero-downtime deployment strategies
                \item Received IQVIA Impact Program – Silver award (May 2025) for outstanding performance and essential feature development
                \item Technologies: C\#, .NET Core, PostgreSQL, MongoDB, AWS, Docker, Kubernetes, Jaeger, OpenTelemetry, GitLab CI, Angular
            \end{highlights}
        \end{onecolentry}
    ...
    \section{Education}
        ...
        \begin{onecolentry}
            \begin{highlights}
                \item GPA: 3.53/4.0 (3.61 in the final term) \end{highlights} 
    ...
    \textbf{StegFormer-MC: Multi-Cover Image Steganography with Distributed Secret Embedding}
\end{twocolentry}

\begin{onecolentry}
    \begin{highlights}
        \item Research collaboration with Professor A. B. M. Alim Al Islam from BUET, specializing in wireless networking, embedded systems, HCI, and security
        \item Extended StegFormer architecture to hide secret images across multiple cover images, reducing per-cover payload and enhancing imperceptibility
        \item Developed channel and spatial-splitting strategies with parallel processing branches and composite loss function for robust training
        \item Achieved high cover-stego PSNR (>40 dB) and secret recovery PSNR (>45 dB) under ideal and perturbed conditions
    \end{highlights}
\end{onecolentry}
